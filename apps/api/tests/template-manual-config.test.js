...
test('manual replacement preserves normalized coarse regions for Image2 semantic guidance', () => {
  const analysis = createManualTemplateAnalysis({
    ...
    protectedRegions: [{ x: 0.45, y: 0.2, width: 0.2, height: 0.08 }]
  });
  ...
  assert.deepEqual(analysis.protected_regions, [{ x: 0.45, y: 0.2, width: 0.2, height: 0.08 }]);
  ...
  assert.deepEqual(copy.protected_regions, []);
